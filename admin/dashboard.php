...
foreach(['pending','in_progress','resolved','closed','rejected'] as $s) {
    $st = $pdo->prepare("SELECT COUNT(*) FROM issues WHERE status=?"); $st->execute([$s]);
    $stats[$s] = $st->fetchColumn();
}
$stmtTotal   = $pdo->query("SELECT COUNT(*) FROM issues"); $stats['total'] = $stmtTotal->fetchColumn();
$stmtUsers   = $pdo->query("SELECT COUNT(*) FROM users WHERE role IN ('student','staff')"); $totalUsers = $stmtUsers->fetchColumn();
$stmtCrit    = $pdo->query("SELECT COUNT(*) FROM issues WHERE priority='critical' AND status NOT IN ('resolved','closed')"); $criticalOpen = $stmtCrit->fetchColumn();
  ...
  <div class="main-content">
    ...
    <div class="page-content">
      ...
      <div class="stat-grid">
        ...
        <div class="stat-card">
          <div class="stat-icon" style="color:var(--blue);border-color:rgba(59,130,246,.2);background:rgba(59,130,246,.06)"><i class="bi bi-tools"></i></div>
        ...
        <div class="stat-card">
          <div class="stat-icon" style="color:var(--rose);border-color:rgba(244,63,94,.2);background:rgba(244,63,94,.06)"><i class="bi bi-lightning"></i></div>
          <div class="stat-value"><?= $criticalOpen ?></div>
        ...
        <div style="display:flex;flex-direction:column;gap:12px;">
          ...
          <div class="panel">
            ...
            <div class="panel-body" style="padding:12px 14px;">
              <?php
              $max = max(array_column($catStats,'cnt') ?: [1]);
              foreach($catStats as $cs):
                $pct = $max > 0 ? round(($cs['cnt']/$max)*100) : 0;
              ?>
              <div style="margin-bottom:10px;">
                <div style="display:flex;justify-content:space-between;margin-bottom:3px;">
                  <span style="font-size:.75rem;font-weight:500;"><?= htmlspecialchars($cs['category_name']) ?></span>
                  <span class="issue-id"><?= $cs['cnt'] ?></span>
                </div>
