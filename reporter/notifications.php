...
$allNotifs->execute([$u['id']]);
$allNotifs = $allNotifs->fetchAll();

$pageTitle = 'Notifications';
$pageSubtitle = 'All your alerts and updates';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <div class="page-content">
      <div class="panel">
        <div class="panel-header">
          ...
          <a href="?mark_all=1" class="btn-glow" style="padding:6px 14px;font-size:0.8rem;text-decoration:none;border-radius:8px;">Mark all read</a>
        </div>
        <?php if(empty($allNotifs)): ?>
          <div class="empty-state"><i class="bi bi-bell-slash"></i><h5>No notifications</h5><p>You're all caught up!</p></div>
        <?php else: ?>
          <?php foreach($allNotifs as $n): 
            $iconMap = ['info'=>'bi-info-circle','success'=>'bi-check-circle','warning'=>'bi-bell','danger'=>'bi-exclamation-circle'];
          ...
          <a href="?read=<?= $n['notification_id'] ?>" class="notif-item <?= $n['is_read']?'':'unread' ?>" style="border-radius:0;display:flex;padding:16px 20px;border-bottom:1px solid rgba(255,255,255,0.04);">
            <div class="notif-icon <?= htmlspecialchars($n['notif_type']) ?>" style="width:40px;height:40px;border-radius:10px;">
              <i class="bi <?= $icon ?>"></i>
            </div>
            <div style="flex:1;margin-left:14px;">
              ...
              <?php if($n['issue_title']): ?>
                <div style="font-size:0.75rem;color:var(--text-primary);margin-top:3px;"><i class="bi bi-link-45deg"></i> <?= htmlspecialchars($n['issue_title']) ?></div>
              <?php endif; ?>
              <div style="font-size:0.73rem;color:var(--text-muted);margin-top:4px;"><?= date('d M Y, h:i A', strtotime($n['created_at'])) ?></div>
            </div>
            <?php if(!$n['is_read']): ?>
              <div style="width:8px;height:8px;background:var(--primary);border-radius:50%;flex-shrink:0;margin-top:6px;"></div>
            <?php endif; ?>
          </a>
          <?php endforeach; ?>
        <?php endif; ?>
