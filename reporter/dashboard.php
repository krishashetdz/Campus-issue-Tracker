...
$stmtTotal    = $pdo->prepare("SELECT COUNT(*) FROM issues WHERE reported_by=?"); $stmtTotal->execute([$u['id']]);
$stmtPending  = $pdo->prepare("SELECT COUNT(*) FROM issues WHERE reported_by=? AND status='pending'"); $stmtPending->execute([$u['id']]);
$stmtProgress = $pdo->prepare("SELECT COUNT(*) FROM issues WHERE reported_by=? AND status='in_progress'"); $stmtProgress->execute([$u['id']]);
$stmtResolved = $pdo->prepare("SELECT COUNT(*) FROM issues WHERE reported_by=? AND status='resolved'"); $stmtResolved->execute([$u['id']]);

$total    = $stmtTotal->fetchColumn();
$pending  = $stmtPending->fetchColumn();
$progress = $stmtProgress->fetchColumn();
$resolved = $stmtResolved->fetchColumn();

// Recent issues
$recentStmt = $pdo->prepare("SELECT i.*, c.category_name FROM issues i LEFT JOIN categories c ON i.category_id=c.category_id WHERE i.reported_by=? ORDER BY i.created_at DESC LIMIT 5");
$recentStmt->execute([$u['id']]);
$recentIssues = $recentStmt->fetchAll();

$pageTitle    = 'Dashboard';
$pageSubtitle = 'Welcome back, ' . explode(' ',$u['name'])[0];
  ...
  <div class="main-content">
    ...
    <div class="page-content">
      ...
      <div class="stat-grid">
        ...
        <div class="stat-card">
          <div class="stat-icon" style="color:var(--amber);border-color:rgba(245,158,11,.2);background:rgba(245,158,11,.06)"><i class="bi bi-clock"></i></div>
          <div class="stat-value"><?= $pending ?></div>
          <div class="stat-label">Pending Review</div>
        </div>
        <div class="stat-card">
          <div class="stat-icon" style="color:var(--blue);border-color:rgba(59,130,246,.2);background:rgba(59,130,246,.06)"><i class="bi bi-arrow-repeat"></i></div>
          <div class="stat-value"><?= $progress ?></div>
          <div class="stat-label">In Progress</div>
        </div>
        <div class="stat-card">
          <div class="stat-icon" style="color:var(--emerald);border-color:rgba(16,185,129,.2);background:rgba(16,185,129,.06)"><i class="bi bi-check-circle"></i></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 240px;gap:14px;">
        ...
        <div class="panel">
          <div class="panel-header">
            <span><i class="bi bi-journal-text me-2"></i>Recent Issues</span>
            <a href="my_issues.php">View all</a>
          </div>
          <?php if(empty($recentIssues)): ?>
            <div class="empty-state">
              <i class="bi bi-inbox"></i>
              <h5>No issues reported yet</h5>
              <p>Click "Report Issue" to submit your first campus issue.</p>
              <a href="report_issue.php" class="btn btn-primary" style="margin-top:12px;display:inline-flex;">Report Issue</a>
            </div>
          <?php else: ?>
          ...
          <div class="panel">
            ...
            <div class="panel-body" style="padding:12px 14px;">
              ...
              <a href="notifications.php" class="quick-action">
                ...
                <?php if($unread > 0): ?><span class="menu-badge" style="margin-left:auto;"><?= $unread ?></span><?php endif; ?>
              </a>
            </div>
          </div>
